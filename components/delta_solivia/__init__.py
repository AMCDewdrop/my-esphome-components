...
DeltaSoliviaComponent = delta_solivia_ns.class_(
    "DeltaSoliviaComponent", uart.UARTDevice, cg.PollingComponent
)
...
CONF_INV_SC_MASTER_GRID_VOLTAGE = "sc_master_grid_voltage"
CONF_INV_SC_MASTER_GRID_FREQUENCY = "sc_master_grid_frequency"
CONF_INV_SC_MASTER_GRID_DC_INJ_CURRENT = "sc_master_grid_dc_injection_current"

CONF_INV_SC_SLAVE_GRID_VOLTAGE = "sc_slave_grid_voltage"
CONF_INV_SC_SLAVE_GRID_FREQUENCY = "sc_slave_grid_frequency"
CONF_INV_SC_SLAVE_GRID_DC_INJ_CURRENT = "sc_slave_grid_dc_injection_current"
...
INVERTER_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_INV_SC_MASTER_GRID_VOLTAGE): sensor.sensor_schema(
            unit_of_measurement=UNIT_VOLT,
        ...
        cv.Optional(CONF_INV_SC_MASTER_GRID_FREQUENCY): sensor.sensor_schema(
            unit_of_measurement=UNIT_HERTZ,
            icon="mdi:sine-wave",
            accuracy_decimals=2,
            device_class=DEVICE_CLASS_FREQUENCY,
        ...
        cv.Optional(CONF_INV_SC_MASTER_GRID_DC_INJ_CURRENT): sensor.sensor_schema(
            unit_of_measurement=UNIT_AMPERE,
            icon="mdi:transmission-tower",
            accuracy_decimals=2,
            device_class=DEVICE_CLASS_CURRENT,
        ...
        cv.Optional(CONF_INV_SC_SLAVE_GRID_VOLTAGE): sensor.sensor_schema(
            unit_of_measurement=UNIT_VOLT,
            icon="mdi:transmission-tower",
            accuracy_decimals=2,
            device_class=DEVICE_CLASS_VOLTAGE,
        ...
        cv.Optional(CONF_INV_SC_SLAVE_GRID_FREQUENCY): sensor.sensor_schema(
            unit_of_measurement=UNIT_HERTZ,
            icon="mdi:sine-wave",
            accuracy_decimals=2,
            device_class=DEVICE_CLASS_FREQUENCY,
        ...
        cv.Optional(CONF_INV_SC_SLAVE_GRID_DC_INJ_CURRENT): sensor.sensor_schema(
            unit_of_measurement=UNIT_AMPERE,
            icon="mdi:transmission-tower",
            accuracy_decimals=2,
            device_class=DEVICE_CLASS_CURRENT,
...
async def to_code(config):
    ...
    if not has_gateway and update_interval < 1000:
        LOGGER.warning(
            "— [Solivia] Component update interval for non-gateway operation shouldn't be lower than 1000ms (is now set to %ums)",
            update_interval,
        )
    ...
    for inverter_config in config[CONF_INVERTERS]:
        ...
        inverter = cg.new_Pvariable(
            inverter_config[CONF_ID], DeltaSoliviaInverter(address, variant)
        )
        ...
        async def make_sensor(field):
            ...
            filter_id = cv.declare_id(sensor.ThrottleAverageFilter)(
                f"{field}_{address}"
            )
